...
import { Package, Building2, Check, Shield, Info } from 'lucide-react'
...
// Zip prefix → city name (major areas)
const ZIP_TO_CITY: Record<string, string> = {
  '100': 'New York', '101': 'New York', '102': 'New York', '103': 'Staten Island', '104': 'Bronx',
  '110': 'New York', '111': 'Long Island City', '112': 'Brooklyn', '113': 'Flushing', '114': 'Jamaica',
  '120': 'Albany', '130': 'Syracuse', '140': 'Buffalo', '144': 'Rochester',
  '150': 'Pittsburgh', '170': 'Harrisburg', '190': 'Philadelphia', '191': 'Philadelphia',
  '200': 'Washington', '202': 'Washington', '203': 'Washington', '210': 'Baltimore', '212': 'Baltimore',
  '220': 'Arlington', '222': 'Arlington', '223': 'Alexandria',
  '270': 'Raleigh', '278': 'Charlotte', '280': 'Charlotte',
  '300': 'Atlanta', '303': 'Atlanta', '310': 'Atlanta',
  '320': 'Jacksonville', '327': 'Orlando', '330': 'Miami', '331': 'Miami', '335': 'Tampa', '336': 'Tampa',
  '370': 'Nashville', '372': 'Nashville', '380': 'Memphis',
  '400': 'Louisville', '403': 'Lexington',
  '430': 'Columbus', '440': 'Cleveland', '450': 'Dayton', '452': 'Cincinnati',
  '460': 'Indianapolis', '461': 'Indianapolis',
  '480': 'Detroit', '481': 'Detroit',
  '500': 'Des Moines', '530': 'Madison', '532': 'Milwaukee', '550': 'Minneapolis', '551': 'St. Paul',
  '600': 'Chicago', '601': 'Chicago', '606': 'Chicago',
  '630': 'St. Louis', '640': 'Kansas City',
  '660': 'Kansas City',
  '700': 'New Orleans', '750': 'Dallas', '751': 'Dallas', '770': 'Houston', '780': 'San Antonio', '787': 'Austin',
  '800': 'Denver', '801': 'Denver', '802': 'Denver',
  '840': 'Salt Lake City', '841': 'Salt Lake City',
  '850': 'Phoenix', '852': 'Phoenix', '853': 'Phoenix',
  '889': 'Reno', '890': 'Las Vegas', '891': 'Las Vegas',
  '900': 'Los Angeles', '901': 'Los Angeles', '906': 'Los Angeles', '910': 'Pasadena', '920': 'San Diego', '921': 'San Diego',
  '940': 'San Francisco', '941': 'San Francisco', '943': 'Palo Alto', '950': 'San Jose', '951': 'San Jose',
  '958': 'Sacramento', '959': 'Sacramento',
  '970': 'Portland', '971': 'Portland',
  '980': 'Seattle', '981': 'Seattle', '984': 'Tacoma',
}
...
function getLocationLabel(zip: string): string {
  const prefix = zip.substring(0, 3)
  const state = ZIP_TO_STATE[prefix]
  const city = ZIP_TO_CITY[prefix]
  if (city && state) return `${city}, ${state}, ${zip}`
  if (state) return `${STATE_NAMES[state] || state}, ${state}, ${zip}`
  return zip
}
...
/** Renders n Package icons in a row with a staggered bounce on card hover */
function BoxIcons({ count }: { count: number }) {
  const containerRef = React.useRef<HTMLSpanElement>(null)

  React.useEffect(() => {
    const container = containerRef.current
    const card = container?.closest('[role="radio"]')
    if (!card || !container) return

    const onEnter = () => {
      const icons = container.querySelectorAll<HTMLElement>('.box-icon')
  ...
  return (
    <span ref={containerRef} className="flex items-center gap-0.5">
      {Array.from({ length: count }, (_, i) => (
        <Package key={i} className="box-icon w-4 h-4 transition-colors duration-200 group-hover:text-primary-700" />
      ))}
    </span>
  )
}
...
const SIZE_ICONS: Record<HomeSizeOption, React.ReactNode> = {
  'studio': <BoxIcons count={1} />,
  '1-bedroom': <BoxIcons count={2} />,
  '2-bedroom': <BoxIcons count={3} />,
  '3-bedroom': <BoxIcons count={4} />,
  '4+-bedroom': <BoxIcons count={5} />,
      ...
      <form onSubmit={handleSubmit} className="animate-slide-up has-sticky-button">
        <div className="space-y-6">
          ...
          {zipFrom && zipTo && (
            <div className="text-center space-y-1">
              <p className="text-xs font-semibold uppercase tracking-widest text-neutral-500">
                Looks like you are moving from
              </p>
              <p className="text-lg sm:text-xl font-semibold text-feedback-success">
                {getLocationLabel(zipFrom)} → {getLocationLabel(zipTo)}
              </p>
            </div>
          )}
          ...
          {priceRange && selectedOption && (
            <div className="max-w-lg mx-auto animate-fade-in space-y-2">
              <div className="bg-secondary-300 rounded-lg p-4 text-center">
                <p className="text-sm text-neutral-800">
                  Typical {selectedOption.label.toLowerCase()} moves cost{' '}
                  <span className="font-semibold">${priceRange.low.toLocaleString()}–${priceRange.high.toLocaleString()}</span>
                </p>
                <p className="text-xs text-neutral-600 mt-1">
                  We&apos;ll find you the best rate from top-rated movers
                </p>
              </div>
              <p className="flex items-start gap-1.5 text-[11px] text-neutral-500 leading-relaxed px-1">
                <Info className="w-3 h-3 mt-0.5 flex-shrink-0" />
                <span>
                  These are typical cost ranges based on national averages for a {selectedOption.label.toLowerCase()} move and are not guaranteed quotes. 
                  Your actual price may vary based on distance, inventory, time of year, and other details discussed with your mover.
                </span>
              </p>
            </div>
          )}
